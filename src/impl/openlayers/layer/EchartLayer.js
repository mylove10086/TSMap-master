...
function EchartLayer(map) {
  ...
  this._createLayerContainer = function () {
    ...
    var container = this._container = EchartLayer.Element.createDiv(this._id, {
      height: size[1] + 'px',
      width: size[0] + 'px',
      top: '0px',
      left: '0px',
...
EchartLayer.Element = {
  ...
  createDiv: function (id, style) {
    ...
    EchartLayer.Element.modifyDomStyle(div, style);
    return div;
  },
  /**
   * 修改dom元素的样式相关属性
  ...
  getDomCenter: function (dom, id) {
    ...
    if (!id) {
      ...
      if (outer) {
        outer_h = EchartLayer.Element.getDomStyle(outer, 'height');
        outer_w = EchartLayer.Element.getDomStyle(outer, 'width');
      } else {
    ...
    if (dom) {
      inner_h = EchartLayer.Element.getDomStyle(dom, 'height');
      inner_w = EchartLayer.Element.getDomStyle(dom, 'width');
    } else {
 ...
 */
EchartLayer.prototype.setVisible = function (visible) {
  this.show(visible);
};
/**
 * 设置图层的可见性
 * @param visible true/false
 */
EchartLayer.prototype.getVisible = function (visible) {
